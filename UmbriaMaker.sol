...
contract UmbriaMaker {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    IUniswapV2Factory public factory;
    address public bar;
    address public umbria;
    address public weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        if (token == weth) {
            uint256 amount = IERC20(token).balanceOf(address(this));
            _safeTransfer(token, factory.getPair(weth, umbria), amount);
            return amount;
        }
        ...
        pair.swap(
            amount0Out,
            amount1Out,
            factory.getPair(weth, umbria),
            new bytes(0)
        );
        return amountOut;
    }

    function _toUMBRIA(uint256 amountIn) internal {
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(weth, umbria));
        (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
        address token0 = pair.token0();
